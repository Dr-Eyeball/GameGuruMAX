...
struct importertype
{
	...
	int defaultstatic;
	int ischaracter;
	float fSceneLocationY;
	float fSceneryDiameter;
	bool bEditAllMesh;
	float fPrevCamX;
	float fPrevCamY;
	float fPrevCamZ;
...
struct visualstype
{
	...
	float SkyCloudThickness;
	float SkyCloudSpeed;

	int iShadowSpotCascadeResolution;
	int iShadowPointResolution;
	int iShadowSpotResolution;
	...
	cStr sTerrainTextures[32];
	cStr sTerrainTexturesName[32];
	cStr sGrassTextures[128]; // would like to use GGGRASS_NUM_SELECTABLE_TYPES, but shouldn't be larger than 128
	cStr sGrassTexturesName[128]; // would like to use GGGRASS_NUM_SELECTABLE_TYPES, but shouldn't be larger than 128

	int iTimeOfday;
	bool bSimulate24Hours;
	float fTimeSpeed;

	float fWeatherIntensity;
	float fWeatherLighting;
	...
	bool bEndableAmbientMusicTrack;
	cStr sAmbientMusicTrack;
	int iAmbientMusicTrackVolume;
	bool bEnableCombatMusicTrack;
	cStr sCombatMusicTrack;
	int iCombatMusicTrackVolume;

	bool bRainEnabled = false;
	float fRainSpeedX = -1.5;
	float fRainSpeedY = -18.0;
	float fRainOpacity = 0.0;
	float fRainScaleX = 1.0;
	float fRainScaleY = 1.0;
	float fRainRefreactionScale = 0.005;

	bool bSnowEnabled = false;
	float fSnowLayers = 15.0;
	float fSnowDepth = 0.5;
	float fSnowWind = 0.5;
	float fSnowSpeed =2.0;
	float fSnowOpacity = 1.0;
	float fSnowOffset = 0.0;

	bool bEndableTreeDrawing = false;
	...
	visualstype ( )
	{
		 ...
		 SunBlue_f = 255.0f;
		 ZenithRed_f = 104.0f;
		 ZenithGreen_f = 104.0f;
		 ZenithBlue_f = 104.0f;

		 fSunShadowBias = 0.00005f;
		 bColorGrading = false;
		 ColorGradingLUT = "";
		 bBloomEnabled = false;
		 ...
		 SkyCloudiness = 0.68f;
		 SkyCloudCoverage = 1.38f;
		 SkyCloudHeight = 60000.0f;
		 SkyCloudThickness = 98000.0f;
		 SkyCloudSpeed = 3.0f;
		 ...
		 iTimeOfday = 1;
		 SunAngleX = 0.0f; //40.0f; calculated in visuals_calcsunanglefromtimeofday
		 SunAngleY = 0.0f; //320.0f;
		 SunAngleZ = 0.0f; //40.0f;

		 bSimulate24Hours = false;
		 fTimeSpeed = 1.0;

		 fWeatherIntensity = 50.0;
		 ...
		 for (int iL = 0; iL < 32; iL++) {
			 sTerrainTextures[iL] = "";
			 sTerrainTexturesName[iL] = "";
		 }
		 for (int iL = 0; iL < 128; iL++) {
			 sGrassTextures[iL] = "";
			 sGrassTexturesName[iL] = "";
		 ...
		 lightShaftWeight = 0.3f;
		 lightShaftDecay = 0.945f;
		 lightShaftExposure = 0.7f;

		 bool bEndableAmbientMusicTrack = false;
		 cStr sAmbientMusicTrack = "";
		 int iAmbientMusicTrackVolume = 100;
		 bool bEnableCombatMusicTrack = false;
		 cStr sCombatMusicTrack = "";
		 int iCombatMusicTrackVolume = 100;

		 bEndableTreeDrawing = false;
...
struct visualsdatastoragetype
{
	...
	visualsdatastoragetype(const visualstype& visuals)
	{
		...
		SunBlue_f = visuals.SunBlue_f;
		ZenithRed_f = visuals.ZenithRed_f;
...
struct WickedMaterial
{
	bool MaterialActive;
	cStr baseColorMapName[MAXMESHMATERIALS];
	cStr normalMapName[MAXMESHMATERIALS];
	cStr surfaceMapName[MAXMESHMATERIALS];
	cStr displacementMapName[MAXMESHMATERIALS];
	cStr emissiveMapName[MAXMESHMATERIALS];
	#ifndef DISABLEOCCLUSIONMAP
	cStr occlusionMapName[MAXMESHMATERIALS];
	#endif
	float fNormal[MAXMESHMATERIALS];
	float fRoughness[MAXMESHMATERIALS];
	float fMetallness[MAXMESHMATERIALS];
	float fEmissive[MAXMESHMATERIALS];
	float fAlphaRef[MAXMESHMATERIALS];
	DWORD dwBaseColor[MAXMESHMATERIALS];
	DWORD dwEmmisiveColor[MAXMESHMATERIALS];
	bool bTransparency[MAXMESHMATERIALS];
	bool bDoubleSided[MAXMESHMATERIALS];
	float fRenderOrderBias[MAXMESHMATERIALS];
...
struct entityprofiletype
{
	...
	entityprofiletype ( )
	{
		 ...
		 thumbnailbackdrop = "";
		 BackBufferZoom = -1.0f;
		 BackBufferCamLeft = -1.0f;
		 BackBufferCamUp = -1.0f;
		 BackBufferRotateX = -1.0f;
		 BackBufferRotateY = -1.0f;
		 iThumbnailAnimset = -1;
		 keywords_s = "";
		 bIsDecal = false;
		 iDecalRows = 0;
...
struct entityeleproftype
{
	...
	int iCharAlliance;
	int iCharFaction;
	int iObjectReserved1;
	int iObjectReserved2;
	int iObjectReserved3;
	int iCharPatrolMode; // Patrol choices
	float fCharRange[2]; // Patrol wait slider (ms)
	float fObjectDataReserved[10];
	int iObjectRelationships[10];
	int iObjectRelationshipsType[10]; //1=char,char 2=char,flag...
	int iObjectRelationshipsData[10]; //if(char,char) 0=Allies 1=Enemies 2=Neutrals ...
	int iOverrideCollisionMode;
	float weapondamagemultiplier;
	float meleedamagemultiplier;
	int iAffectedByGravity;
	int iMoveSpeed;
	int iTurnSpeed;
	int iFlattenID;
	bool bAutoFlatten;
	...
	entityeleproftype ( )
	{
		...
		for (int i = 0;i < 10;i++)
		{
			fObjectDataReserved[i] = 0;
			iObjectRelationships[i] = 0;
			iObjectRelationshipsType[i] = 0;
			iObjectRelationshipsData[i] = 0;
		}
		iAffectedByGravity = 1;

		newparticle.bParticle_Preview = true;
		newparticle.bParticle_Show_At_Start = true;
		newparticle.bParticle_Looping_Animation = true;
		newparticle.bParticle_Full_Screen = false;
		newparticle.fParticle_Fullscreen_Duration = 10.0f; //Sec.
		newparticle.fParticle_Fullscreen_Fadein = 1.0f; //Sec.
		newparticle.fParticle_Fullscreen_Fadeout = 1.0f; //Sec.
		newparticle.Particle_Fullscreen_Transition = "";
		newparticle.fParticle_Speed = 1.0f;
		newparticle.fParticle_Opacity = 1.0f;
		iOverrideCollisionMode = -1;
...
struct entityluastatestype
{
	...
	cstr returningaimain_s;
	int returningaimainstored;
...
struct entitytype
{
	...
	int ragdollplusactivate;
	int ragdollplusweapontypeused;
	int ragdollifiedplusphyobj;
	int editorlock;
	cstr overprompt_s;
	...
	int soundset5;
	int soundset6;
	float firespotlimb_lastx;
	float firespotlimb_lasty;
	float firespotlimb_lastz;
	int quatmode;
	float quatx;
	float quaty;
	float quatz;
	...
	entitytype ( )
	{
		 ...
		 overprompt_s = "";
		 editorlock = 0;
		 ragdollplusactivate = 0;
		 ragdollplusweapontypeused = 0;
		 ragdollifiedplusphyobj = 0;
		 ...
		 soundset5 = 0;
		 soundset6 = 0;
		 firespotlimb_lastx = -1.0;
		 firespotlimb_lasty = -1.0;
		 firespotlimb_lastz = -1.0;
		 quatmode = 0; //PE: Only wicked :)
		 quatx = 0;
		 quaty = 0;
		 quatz = 0;
		 quatw = 1;
...
struct charanimstatetype
{
	...
	float movetoz_f;
	int movingbackward;
	int requiremovementnow;
	float movespeed_f;
	float turnspeed_f;
	float moveangle_f;
	float remainingMoveDistanceOnPath_f;
	float remainingOverallDistanceToDest_f;
	int updatemoveangle;
	int runmode;
	int strafemode;
	int ammoinclipmax;
	...
	float neckUpAndDown;
	float neckRightAndLeftLimit;
	float neckRightAndLeftOffset;
	float neckUpAndDownLimit;
	float neckUpAndDownOffset;
	float spineAiming;
	float spineRightAndLeft;
	float spineRightAndLeftLimit;
	float spineRightAndLeftOffset;
	float spineUpAndDownLimit;
	float spineUpAndDownOffset;
	float spineYAdjust;
	float spineZAdjust;
	...
	int iTiltMode;
	int iStopFromEnd;
	int iRotationAlongPathMode;

	// Constructor
	charanimstatetype ( )
	{
		 iRotationAlongPathMode = 100;
		 iStopFromEnd = 10;
		 iTiltMode = 0;
		 memset(pointx, 0, sizeof(pointx));
		 memset(pointy, 0, sizeof(pointy));
		 memset(pointz, 0, sizeof(pointz));
		 ...
		 spineRightAndLeftLimit = 45;
		 spineRightAndLeftOffset = 0;
		 ...
		 movespeed_f = 1.0f;
		 movingbackward = 0;
		 requiremovementnow = 0;
		 remainingMoveDistanceOnPath_f = 0.0f;
		 remainingOverallDistanceToDest_f = 0.0f;
		 turnspeed_f = 10.0f;
		 moveangle_f = 0.0f;
		 lastcovering = 0;
		 lastmoving = 0;
...
struct playercontroltype
{
	...
	playercontroltype ( )
	{
		 iPlayHeartBeatSoundOff = 1;
		 iShowScreenBloodOff = 1;
		 canrun = 1;
		 gameloopinitflag = 0;
...
struct playerlighttype
{
	...
	float flashlightcontrol_range_f;
	float flashlightcontrol_radius_f;
	float flashlightcontrol_colorR_f;
	float flashlightcontrol_colorG_f;
	...
	playerlighttype ( )
	{
		 flashlightcontrol_range_f = 45;
		 flashlightcontrol_radius_f = 700;
		 flashlightcontrol_colorR_f = 64;
