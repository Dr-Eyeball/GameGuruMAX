...
DARKSDK_DLL void UpdateOverlayFlag ( sObject* pObject )
{
	...
	#ifdef WICKEDENGINE
	extern void WickedCall_SetObjectDisableDepth (sObject*, bool);
	WickedCall_SetObjectDisableDepth(pObject, pObject->bNewZLayerObject);
	#endif
}
...
DARKSDK_DLL bool SetNewObjectFinalProperties ( int iID, float fRadius )
{
	...
	#ifdef WICKEDENGINE
	// for wicked, need basic texture structure for each mesh so wicked can fill it later
	for ( int iMesh = 0; iMesh < pObject->iMeshCount; iMesh++ )
	{
		sMesh* pMesh = pObject->ppMeshList [ iMesh ];
		if (pMesh && pMesh->pTextures == NULL)
		{
	...
	// Ensure wicked engine is aware of new object
	#ifdef WICKEDENGINE
	//PE: InstanceObject - setup instancing index.
	extern int iUseMasterObjectID;
	if (iUseMasterObjectID > 0)
	{
		extern bool bUseInstancing;
		if (bUseInstancing)
		{
			sObject* pNewObject = pObject;
			sObject* pMasterObject = g_ObjectList[iUseMasterObjectID];
			DWORD dwMeshCount = pMasterObject->iMeshCount;
			if (pNewObject->iMeshCount < (int)dwMeshCount) dwMeshCount = pNewObject->iMeshCount;
			for (int iMesh = 0; iMesh < (int)dwMeshCount; iMesh++)
			{
				sMesh* pMesh = NULL; if (iMesh < pMasterObject->iMeshCount) pMesh = pMasterObject->ppMeshList[iMesh];
				sMesh* pNewMesh = NULL; if (iMesh < pNewObject->iMeshCount) pNewMesh = pNewObject->ppMeshList[iMesh];
				if (pMesh && pNewMesh)
				{
					if (bUseInstancing)
					{
						pNewMesh->master_wickedmeshindex = pMesh->wickedmeshindex; //PE: Instancing get master meshindex.
						pNewMesh->master_wickedmaterialindex = pMesh->wickedmaterialindex;
						pNewMesh->master_wickedarmatureindex = pMesh->wickedarmatureindex;
						pNewMesh->bInstanced = true;
					}
					else
					{
						pNewMesh->master_wickedmeshindex = 0; //PE: Instancing get master meshindex.
						pNewMesh->master_wickedmaterialindex = 0;
						pNewMesh->master_wickedarmatureindex = 0;
						pNewMesh->bInstanced = false;
					}
				}
			}
		}
